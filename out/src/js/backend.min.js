...
return d=d.replace(/<img[^>]*src=\s*"([^"]+)"[^>]*data-filename=\s*"([^">]+)"[^>]*class=\s*"[^">]*dd-wysiwyg-media-image[^">]*"[^>]*>/gi,function(a,b,c){return a.replace(b,"[{$oViewConf->getMediaUrl()}]"+c)})}})}(jQuery),/**
...
if("function"==typeof a().summernote){a(".ddoe-wysiwyg-editor > textarea").each(function(){a(this).closest("td").find(".messagebox").remove();var b=a(this).height(),c=a(this).summernote({lang:"de-DE",minHeight:b||100,toolbar:[["style",["style"]],["formatting",["bold","italic","underline","strikethrough","clear"]],
//[ 'fontname', [ 'fontname' ] ],
["fontsize",["fontsize"]],["color",["color"]],["layout",["ul","ol","paragraph"]],["height",["height"]],["table",["table"]],["insert",["link","video","hr"]],["misc",["codeview"]]],dialogsInBody:!1,buttons:{ddmedia:"ddmedia"},disableDragAndDrop:!0});"disabled"===a(this).attr("disabled")&&a(this).summernote("disable");var d=c.data("summernote");d.invoke("codeview.activate"),d.invoke("codeview.deactivate")});var b=a(".ddoe-wysiwyg-editor").first().closest("form");b.find('*[type="submit"]').first().on("click",function(){a(".ddoe-wysiwyg-editor > textarea",b).each(function(){var b=a(this).data("summernote");
// deactivate codeview before getting value
b.invoke("codeview.isActivated")&&b.invoke("codeview.deactivate"),b.invoke("codeview.activate")})})}})}(jQuery);
